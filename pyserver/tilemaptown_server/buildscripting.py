...
def fn_e_step(e, arg): #Ei
	...
	if e.has_permission(e2, perm=permission['move']):
		...
		if e2.map and new_x >= 0 and new_y >= 0 and new_x < e2.map.width and new_y < e2.map.height and (not get_tile_density(e2.map.turfs[new_x][new_y]) and not any((get_tile_density(o) for o in (e2.map.objs[new_x][new_y] or [])))):
